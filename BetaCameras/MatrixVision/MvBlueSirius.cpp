...
namespace MetriCam2
{
	namespace Cameras
	{
		...
		IProjectiveTransformation^ MvBlueSirius::GetIntrinsics(String^ channelName)
		{
			...
			{
				return gcnew ProjectiveTransformationRational(_depthRaw->Width, _depthRaw->Height, FocalLength, FocalLength, _depthRaw->Width / 2.0f, _depthRaw->Height / 2.0f, 0, 0, 0, 0, 0, 0, 0, 0);
			}
			if (MetriCam2::ChannelNames::Color == channelName)
			{
				return gcnew ProjectiveTransformationRational(_color->Width, _color->Height, FocalLength, FocalLength, _color->Width / 2.0f, _color->Height / 2.0f, 0, 0, 0, 0, 0, 0, 0, 0);
			}
